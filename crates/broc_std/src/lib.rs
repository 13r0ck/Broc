...
pub use broc_box::BrocBox;
pub use broc_dict::BrocDict;
pub use broc_list::{BrocList, SendSafeBrocList};
pub use broc_set::BrocSet;
pub use broc_str::{InteriorNulError, BrocStr, SendSafeBrocStr};
pub use storage::Storage;

// A list of C functions that are being imported
extern "C" {
    pub fn broc_alloc(size: usize, alignment: u32) -> *mut c_void;
    pub fn broc_realloc(
        ptr: *mut c_void,
        new_size: usize,
        old_size: usize,
    ...
    pub fn broc_dealloc(ptr: *mut c_void, alignment: u32);
    pub fn broc_panic(c_ptr: *mut c_void, tag_id: u32);
    pub fn broc_memcpy(dst: *mut c_void, src: *mut c_void, n: usize) -> *mut c_void;
    pub fn broc_memset(dst: *mut c_void, c: i32, n: usize) -> *mut c_void;
}

pub fn broc_alloc_refcounted<T>() -> *mut T {
    let size = core::mem::size_of::<T>();
    let align = core::mem::align_of::<T>();

    broc_alloc_refcounted_help(size, align) as *mut T
}

fn broc_alloc_refcounted_help(mut size: usize, mut align: usize) -> *mut u8 {
    let prefix = if align > 8 { 16 } else { 8 };
    size += prefix;
    align = align.max(core::mem::size_of::<crate::Storage>());

    unsafe {
        let allocation_ptr = broc_alloc(size, align as _) as *mut u8;
...
/// Like a Rust `Result`, but following Broc's ABI instead of Rust's.
/// (Using Rust's `Result` instead of this will not work properly with Broc code!)
///
/// This can be converted to/from a Rust `Result` using `.into()`
#[repr(C)]
pub struct BrocResult<T, E> {
    payload: BrocResultPayload<T, E>,
    tag: BrocResultTag,
}

impl<T, E> Debug for BrocResult<T, E>
where
    T: Debug,
    E: Debug,
{
...
impl<T, E> PartialEq for BrocResult<T, E>
where
    T: PartialEq,
    E: PartialEq,
{
...
impl<T, E> Clone for BrocResult<T, E>
where
    T: Clone,
    E: Clone,
{
    fn clone(&self) -> Self {
        match self.as_result_of_refs() {
            Ok(payload) => BrocResult::ok(ManuallyDrop::into_inner(payload.clone())),
            Err(payload) => BrocResult::err(ManuallyDrop::into_inner(payload.clone())),
        }
    }
}

impl<T, E> BrocResult<T, E> {
    pub fn ok(payload: T) -> Self {
        Self {
            tag: BrocResultTag::BrocOk,
            payload: BrocResultPayload {
    ...
    pub fn is_ok(&self) -> bool {
        matches!(self.tag, BrocResultTag::BrocOk)
    }

    pub fn is_err(&self) -> bool {
        matches!(self.tag, BrocResultTag::BrocErr)
    }

    fn into_payload(self) -> BrocResultPayload<T, E> {
    ...
    fn as_result_of_refs(&self) -> Result<&ManuallyDrop<T>, &ManuallyDrop<E>> {
        ...
        unsafe {
            match self.tag {
                BrocOk => Ok(&self.payload.ok),
                BrocErr => Err(&self.payload.err),
            }
        }
    }
}

impl<T, E> From<BrocResult<T, E>> for Result<T, E> {
    fn from(broc_result: BrocResult<T, E>) -> Self {
        use BrocResultTag::*;

        let tag = broc_result.tag;
        let payload = broc_result.into_payload();

        unsafe {
            match tag {
                BrocOk => Ok(ManuallyDrop::into_inner(payload.ok)),
                BrocErr => Err(ManuallyDrop::into_inner(payload.err)),
            }
        }
    }
}

impl<T, E> From<Result<T, E>> for BrocResult<T, E> {
    fn from(result: Result<T, E>) -> Self {
        match result {
            Ok(payload) => BrocResult::ok(payload),
            Err(payload) => BrocResult::err(payload),
        }
    }
}
...
#[repr(C)]
union BrocResultPayload<T, E> {
    ok: ManuallyDrop<T>,
    err: ManuallyDrop<E>,
}

impl<T, E> Drop for BrocResult<T, E> {
    fn drop(&mut self) {
        use BrocResultTag::*;

        match self.tag {
            BrocOk => unsafe { ManuallyDrop::drop(&mut self.payload.ok) },
            BrocErr => unsafe { ManuallyDrop::drop(&mut self.payload.err) },
        }
    }
}
...
impl BrocDec {
    ...
    /// This is private because BrocDec being an i128 is an implementation detail
    #[inline(always)]
    ...
    fn to_str_helper(self, string: &mut ArrayString<{ Self::MAX_STR_LENGTH }>) -> &str {
        ...
        static_assertions::const_assert!(BrocDec::DECIMAL_PLACES + 1 == 19);
    ...
    pub fn to_str(&self) -> BrocStr {
        BrocStr::from(self.to_str_helper(&mut ArrayString::new()))
    }
}

impl fmt::Display for BrocDec {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
