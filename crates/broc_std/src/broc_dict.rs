...
/// At the moment, Broc's Dict is just an association list. Its lookups are O(n) but
...
/// memory is managed in Broc:
///
/// 1. We don't implement an [`IntoIterator`] that iterates over owned values,
///    since Broc owns the memory, not rust.
/// 2. We use a union for [`BrocDictItem`] instead of just a struct. See the
///    comment on that data structure for why.
#[derive(Default, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
#[repr(transparent)]
pub struct BrocDict<K, V>(BrocList<BrocDictItem<K, V>>);

impl<K, V> BrocDict<K, V> {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self(BrocList::with_capacity(capacity))
    }
...
impl<K: Hash, V> BrocDict<K, V> {
    unsafe fn insert_unchecked(&mut self, _key: K, _val: V) {
        todo!();
    }
}

impl<K: Hash, V> FromIterator<(K, V)> for BrocDict<K, V> {
...
impl<'a, K, V> IntoIterator for &'a BrocDict<K, V> {
    type Item = (&'a K, &'a V);
...
impl<K: Debug, V: Debug> Debug for BrocDict<K, V> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("BrocDict ")?;
...
/// Broc is constructing these values according to its memory layout rules.
/// Specifically:
///
...
impl<K, V> Drop for BrocDictItem<K, V> {
    fn drop(&mut self) {
        if align_of::<K>() >= align_of::<V>() {
...
impl<K: PartialEq, V: PartialEq> PartialEq for BrocDictItem<K, V> {
    fn eq(&self, other: &Self) -> bool {
        self.key() == other.key() && self.value() == other.value()
    }
}

impl<K: PartialOrd, V: PartialOrd> PartialOrd for BrocDictItem<K, V> {
...
impl<K: Ord, V: Ord> Ord for BrocDictItem<K, V> {
...
impl<K: Hash, V: Hash> Hash for BrocDictItem<K, V> {
