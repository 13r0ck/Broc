...
use crate::{broc_alloc, broc_dealloc, broc_realloc, storage::Storage};
...
impl<T> BrocList<T> {
    ...
    /// Create an empty BrocList with enough space preallocated to store
    ...
    /// Used for both broc_alloc and broc_realloc - given the number of elements,
    ...
    fn elems_with_capacity(num_elems: usize) -> NonNull<ManuallyDrop<T>> {
        let alloc_ptr = unsafe { broc_alloc(Self::alloc_bytes(num_elems), Self::alloc_alignment()) };

        Self::elems_from_allocation(NonNull::new(alloc_ptr).unwrap_or_else(|| {
            todo!("Call broc_panic with the info that an allocation failed.");
        }))
    }
    ...
    ///
    /// This is because BrocList values are not allocated using the system allocator, so
        ...
        let non_null_elements = if let Some((elements, storage)) = self.elements_and_storage() {
            ...
            if is_unique {
                ...
                if self.capacity() >= slice.len() {
                    ...
                    // Since this is a unique BrocList, we can use realloc here.
                    let new_ptr = unsafe {
                        broc_realloc(
                            storage.as_ptr().cast(),
...
impl<T> BrocList<T> {
    /// Increase a BrocList's capacity by at least the requested number of elements (possibly more).
    ///
    /// May return a new BrocList, if the provided one was not unique.
            ...
            Some((elements, storage)) => {
                if storage.get().is_unique() {
                    unsafe {
                        ...
                        // so we don't need to call broc_dealloc here.
                    }
                } else {
                    ...
                    if !new_storage.is_readonly() {
                        ...
                        if needs_dealloc {
                            ...
                            unsafe {
                                broc_dealloc(self.ptr_to_allocation(), Self::alloc_alignment());
                            }
                        } else {
...
impl<T> DerefMut for BrocList<T> {
...
impl<T> Default for BrocList<T> {
    fn default() -> Self {
        Self::empty()
    }
}

impl<T, U> PartialEq<BrocList<U>> for BrocList<T>
where
    T: PartialEq<U>,
{
    fn eq(&self, other: &BrocList<U>) -> bool {
        self.deref() == other.deref()
    }
}

impl<T> Eq for BrocList<T> where T: Eq {}

impl<T, U> PartialOrd<BrocList<U>> for BrocList<T>
where
    T: PartialOrd<U>,
{
    fn partial_cmp(&self, other: &BrocList<U>) -> Option<cmp::Ordering> {
...
impl<T, const SIZE: usize> From<[T; SIZE]> for BrocList<T> {
    fn from(array: [T; SIZE]) -> Self {
        Self::from_iter(array)
    }
}

impl<'a, T> IntoIterator for &'a BrocList<T> {
    type Item = &'a T;
...
impl<T: Hash> Hash for BrocList<T> {
...
impl<T> FromIterator<T> for BrocList<T> {
...
#[cfg(feature = "serde")]
impl<T: Serialize> Serialize for BrocList<T> {
...
#[cfg(feature = "serde")]
impl<'de, T> Deserialize<'de> for BrocList<T>
where
    ...
    T: Deserialize<'de> + core::clone::Clone,
{
    ...
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_seq(BrocListVisitor::new())
    }
}

// This is a BrocList that is checked to ensure it is unique or readonly such that it can be sent between threads safely.
#[repr(transparent)]
pub struct SendSafeBrocList<T>(BrocList<T>);

unsafe impl<T> Send for SendSafeBrocList<T> where T: Send {}

impl<T> Clone for SendSafeBrocList<T>
where
    T: Clone,
{
    fn clone(&self) -> Self {
        if self.0.is_readonly() {
            SendSafeBrocList(self.0.clone())
        } else {
            // To keep self send safe, this must copy.
            SendSafeBrocList(BrocList::from_slice(&self.0))
        }
    }
}

impl<T> From<BrocList<T>> for SendSafeBrocList<T>
where
    T: Clone,
{
    fn from(l: BrocList<T>) -> Self {
        if l.is_unique() || l.is_readonly() {
            ...
            SendSafeBrocList(BrocList::from_slice(&l))
        }
    }
}

impl<T> From<SendSafeBrocList<T>> for BrocList<T> {
    fn from(l: SendSafeBrocList<T>) -> Self {
        l.0
    }
}

#[cfg(feature = "serde")]
struct BrocListVisitor<T> {
    marker: PhantomData<T>,
}

#[cfg(feature = "serde")]
impl<T> BrocListVisitor<T> {
    fn new() -> Self {
        BrocListVisitor {
            marker: PhantomData,
        }
...
#[cfg(feature = "serde")]
impl<'de, T> Visitor<'de> for BrocListVisitor<T>
where
    T: Deserialize<'de> + core::clone::Clone,
{
    type Value = BrocList<T>;
        ...
        A: serde::de::SeqAccess<'de>,
    {
        let mut out = match seq.size_hint() {
            Some(hint) => BrocList::with_capacity(hint),
            None => BrocList::empty(),
        };
