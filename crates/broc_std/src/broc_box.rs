...
use crate::{broc_alloc, broc_dealloc, storage::Storage};
use core::{
    cell::Cell,
    cmp::{self, Ordering},
    fmt::Debug,
...
impl<T> BrocBox<T> {
    pub fn new(contents: T) -> Self {
        ...
        let ptr = unsafe { broc_alloc(bytes, alignment as u32) };
...
impl<T, U> PartialEq<BrocBox<U>> for BrocBox<T>
where
    T: PartialEq<U>,
{
    fn eq(&self, other: &BrocBox<U>) -> bool {
        self.deref() == other.deref()
    }
}

impl<T> Eq for BrocBox<T> where T: Eq {}

impl<T, U> PartialOrd<BrocBox<U>> for BrocBox<T>
where
    T: PartialOrd<U>,
{
    fn partial_cmp(&self, other: &BrocBox<U>) -> Option<cmp::Ordering> {
...
impl<T: core::hash::Hash> core::hash::Hash for BrocBox<T> {
    fn hash<H: core::hash::Hasher>(&self, state: &mut H) {
        self.contents.hash(state)
    }
}

impl<T> Debug for BrocBox<T>
where
    T: Debug,
{
